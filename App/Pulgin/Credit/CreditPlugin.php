...
class CreditPlugin implements MooPlugin{
    public function install(){
        ...
        //mail
        $request_withdraw_payment_subject = 'Your withdrawal request is being processed';
        $request_withdraw_payment_content = '
        <p>[header]</p>
            <p>Your withdrawal request is being processed.</p>
            <p>Please be reminded that the withdrawal request is processed within 3 - 5 working days. For any inquiries, please don\'t hestitat to contact us.</p>
        <p>[footer]</p>
        ';

        $this->createMailTemplate('request_withdraw_payment', '', $request_withdraw_payment_subject, $request_withdraw_payment_content);
    }
    public function uninstall(){
        ...
        //delete mail withdraw
        $mailModel = MooCore::getInstance()->getModel('Mail.Mailtemplate');
        $list_mail_credit = $mailModel->find('list', array('conditions' => array('Mailtemplate.plugin' => 'Credit')));

        $i18nModel->deleteAll(array(
            'I18nModel.model' => "Mailtemplate",
            "I18nModel.foreign_key" => $list_mail_credit
        ));

        $mailModel->deleteAll(array(
            'Mailtemplate.plugin' => "Credit"
        ));
    }
    ...
    public function menu()
    {
        return array(
            ...
            __d('credit', 'Manage Transactions') => array('plugin' => 'credit', 'controller' => 'credit_transactions', 'action' => 'admin_index'),
            ...
            __d('credit', 'Manage Withdrawal Requests') => array('plugin' => 'credit', 'controller' => 'credit_withdraw_payments', 'action' => 'admin_index')
        );
    }
