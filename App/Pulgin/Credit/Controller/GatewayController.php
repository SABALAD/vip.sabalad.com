...
class GatewayController extends CreditAppController{
	...
	public function process($type = null,$id = null)
	{
		...
		if($this->request->is('post') &&  $credit >= $amount)
		{
			...
			$this->loadModel("Credit.CreditExchanges");
			$this->CreditExchanges->save(array(
				'type' => strtolower($type),
				'type_id' => $id,
				'exchange' => Configure::read('Credit.credit_currency_exchange'),
			));

			$helperPlugin->onSuccessful($item,array('user_id'=>$uid,'amount'=>$params['amount'],'type'=>strtolower($type),'id'=>$id),$params['amount']);
