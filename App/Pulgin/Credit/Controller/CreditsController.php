...
class CreditsController extends CreditAppController
{
    ...
    public function index($type = "")
    {
        ...
        switch ($type) {
            ...
            case 'my_withdraw_requests':
                $this->set('my_withdraw_requests', 'current');
                $this->set('title', __d('credit', 'My withdrawal requests'));
                $this->my_withdraw_requests();
                break;
    ...
    public function my_withdraw_requests(){
        ...
        $cond = array();
        $cond['CreditWithdrawPayment.user_id'] = $uid;
    ...
    private function my_credits()
    {
        ...
        $this->loadModel('Credit.CreditLogs');
        $conditions = array(
            'CreditLogs.user_id' => $uid,
            'CreditActiontypes.is_active' => 1
        );
        $join = array();
        $join[] = array(
            "table" => $this->tablePrefix . "credit_actiontypes",
