            ...
            <p>  <?php echo __d('credit', 'Current Balance')?>
                <em>
                    <?php if(!empty($balance)){
                        echo round($balance['CreditBalances']['current_credit'], 8);
                    }
                    else{ echo '0';}?>
                </em>
            </p>
        ...
        <?php if(!empty($sells)):?>
            <?php foreach ($sells as $sell): ?>
                <div class="sell_item" style="margin-bottom: 10px;">
                    <input type="radio" name="sell_selected" id="sell_selected_<?php echo $sell['CreditSells']['id'];?>" value="<?php echo $sell['CreditSells']['id'].'_'.$sell['CreditSells']['price'].'_'.$sell['CreditSells']['credit'] ?>" /> <label style="display: inline;" for="sell_selected_<?php echo $sell['CreditSells']['id'];?>"> <?php echo round($sell['CreditSells']['credit'], 8) .' '. __d('credit', 'Credits for') . ' '.$currency['Currency']['symbol']. number_format($sell['CreditSells']['price'], 2) ?> </label>
                </div>
            <?php endforeach; ?>
        <?php else:?>
            <div class="clear text-center"> <?php echo __d('credit',  'No more credit package found') ?> </div>
        ...
        <?php foreach($gateways as $gateway):
            ...
            if ($helper->checkSupportCurrency($currency['Currency']['currency_code'])):
                ...
                <form id="formGateway" method="post" action="<?php echo $this->request->base;?>/credits/purchase_credit/<?php echo lcfirst($gateway['name']);?>">
                    ...
                    <input type="submit" class="btn btn-action btnGateway" value="<?php echo __d('credit', 'Pay with '). $gateway['name'];?>" />
                    <br/><br/>
                </form>
            <?php endif;?>
        <?php endforeach;?>
