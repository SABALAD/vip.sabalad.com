    ...
    <div class="content_center">
        ...
        <div class="modal-body">
            ...
            <!-- <form id="buyCreditForm" action="<?php if(Configure::read('Credit.credit_test_mode')):?>https://www.sandbox.paypal.com/cgi-bin/webscr<?php else:?>https://www.paypal.com/cgi-bin/webscr<?php endif;?>" enctype="application/x-www-form-urlencoded"> -->
                <div id="buyCreditForm">
                    <?php if(!empty($sells)):?>
                        <?php foreach ($sells as $sell): ?>
                                <div class="sell_item" style="margin-bottom: 10px;">
                                    <input type="radio" name="sell_selected" id="sell_selected_<?php echo $sell['CreditSells']['id'];?>" value="<?php echo $sell['CreditSells']['id'].'_'.$sell['CreditSells']['price'].'_'.$sell['CreditSells']['credit'] ?>" /> <label style="display: inline;" for="sell_selected_<?php echo $sell['CreditSells']['id'];?>">
                                    <?php echo number_format($sell['CreditSells']['credit'], 0) .' '. __d('credit', 'Credits for') . ' '.$currency['Currency']['symbol']. number_format($sell['CreditSells']['price'], 2) ?></label>
                                </div>
                        <?php endforeach; ?>    
                    <?php else:?>
                    ...
                    <?php if(Configure::read('Credit.credit_paypal_email')):?>
                        <div>
                            <form action="<?php if(Configure::read('Credit.credit_test_mode')):?>https://www.sandbox.paypal.com/cgi-bin/webscr<?php else:?>https://www.paypal.com/cgi-bin/webscr<?php endif;?>" enctype="application/x-www-form-urlencoded">
                                <div>
                                    <input type="submit" class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--raised mdl-button--colored1" id="btnPaypal" value="<?php echo __d('credit',  'Pay with PayPal')?>" >
                                </div>
                    ...
                    <div>
                        <?php foreach($gateways as $gateway):
                            ...
                            if ($helper->checkSupportCurrency($currency['Currency']['currency_code'])):
                                ?>
                                <form id="formGateway" method="post" action="<?php echo $this->request->base;?>/credits/purchase_credit/<?php echo lcfirst($gateway['name']);?>?app_no_tab=1">
                                    ...
                                    <input type="submit" class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--raised mdl-button--colored1 btnGateway" value="<?php echo __d('spotlight', 'Pay with %s', $gateway['name']);?>" />
                                    <br/><br/>
                                </form>
                            <?php endif;?>
                        <?php endforeach;?>
