...
//            $credit_balances->id = $user_id;
    ...
    public function refund($params)
    {
        ...
        $type = '';
        $id = '';
        $exchange = Configure::read('Credit.credit_currency_exchange');
        if (isset($params['type']))
        {
        	...
        	$exchangeModel = MooCore::getInstance()->getModel("Credit.CreditExchanges");
        	$result = $exchangeModel->findByTypeAndTypeId($type,$id);
        	if ($result)
        	{
        		$exchange = $result['CreditExchanges']['exchange'];
        	}
        }        
        $amount = round($amount*$exchange,1);
    ...
    public function getParamsPayment($item) {
        $creditOrders = (isset($item['CreditOrder']) ? $item['CreditOrder'] : array());
        ...
        $params = array(
            ...
            'id' => (isset($creditOrders['id']) ? $creditOrders['id'] : 0),
            'amount' => (isset($creditOrders['price']) ? $creditOrders['price'] : 0),
            'total_amount' => (isset($creditOrders['price']) ? $creditOrders['price'] : 0),
        );
        return $params;
    }
    public function onFailure($item, $data) {
        $cTransModel = MooCore::getInstance()->getModel('Credit.CreditOrder');

        $data = array(
            'status' => 'failed'
        );
        $cTransModel->id = $item['CreditOrder']['id'];
