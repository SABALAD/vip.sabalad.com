...
class CreditListener implements CakeEventListener
{
    ...
    public function implementedEvents()
    {
        return array(
            ...
            'Controller.Plugin.afterUninstall' => 'afterUninstall'
        );
    }
    ...
    {   
        ...
        // check user id when admin  approved item
        if(isset($model->data[$model->name]['user_id']) && !empty($model->data[$model->name]['user_id'])){
            $uid = $model->data[$model->name]['user_id'];
        }
        ...
        $fileds = $model->getColumnTypes();
        $add_credit = false;
       
        if(array_key_exists('approved_credit', $fileds)) {

           if(isset($model->data[$model->name]['approved_credit']) && $model->data[$model->name]['approved_credit'] == 1){
                $add_credit = true;
           }

           if(isset($model->data[$model->name]['approved_credit']) && $model->data[$model->name]['approved_credit'] == 0){
                $this->doBeforeDelete($event);
                return;
           }
        ...
        if ( $model->name && $add_credit) {
            ...
            if($type == 'comment' && $model->data['Comment']['type'] == 'core_activity_comment'){
    ...
    {   
        ...
        if(array_key_exists('approved_credit', $fileds)) {
            ...
            if(!empty($model->id) && empty($delete_credit)){
                $info_item = $model->findById($model->id);
                if($info_item[$model->name]['approved_credit'] == 1){
                    $delete_credit = true;
                }
            }
        ...
        if ($model->name && $delete_credit) {
            ...
            // // check user id when admin  approved item or remove
            if(isset($model->data[$model->name]['user_id']) && !empty($model->data[$model->name]['user_id'])){
            $uid = $model->data[$model->name]['user_id'];
            }
    ...
    public function beforeRender($event)
    {
        if (Configure::read('Credit.credit_enabled')) {
            ...
            $e->addPhraseJs(array(
                ...
                'are_you_sure_you_want_to_remove_this_entry' => __d('credit', 'Are you sure you want to remove this entry?')
            ));
    ...
    public function afterUninstall($e){
        $settingModel = MooCore::getInstance()->getModel('Setting');
        $settingModel->save_boot_settings();
    }
}
